...
function pb_call_gemini(array $conversationHistory, string $apiKey, array $models, int $maxAttempts, array $retryable): array
{
    $data = [
...
for ($jsonTry = 0; $jsonTry <= $maxJsonRetries; $jsonTry++) {
    ...
    $result = pb_call_gemini($conversationHistory, $api_key, $gemini_models, $maxAttemptsPerModel, $retryableCodes);
...
if ($title === '' || $bodyText === '') {
    ...
    $result2 = pb_call_gemini($conversationHistory, $api_key, $gemini_models, $maxAttemptsPerModel, $retryableCodes);
    if ($result2['http'] === 200) {
