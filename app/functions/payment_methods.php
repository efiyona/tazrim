...
function tazrim_payment_method_public_name(array $method): string {
    ...
    $last4 = preg_replace('/\D+/', '', (string)($method['last4'] ?? ''));
    if ($last4 !== '' && preg_match('/' . preg_quote($last4, '/') . '\s*$/u', $name)) {
        $name = preg_replace('/[\s\-–—·•]*' . preg_quote($last4, '/') . '\s*$/u', '', $name);
    }
    return trim($name) !== '' ? trim($name) : 'אמצעי תשלום';
}
