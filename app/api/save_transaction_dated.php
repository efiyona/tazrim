...
// תאריך אופציונלי מהקליינט בפורמט Y-m-d. אם לא נשלח - ברירת מחדל היום (כמו save_transaction.php).
$transaction_date_raw = trim((string) ($_POST['transaction_date'] ?? ''));
if ($transaction_date_raw === '') {
    $transaction_date = date('Y-m-d');
} else {
    $dt = DateTime::createFromFormat('Y-m-d', $transaction_date_raw);
    if ($dt && $dt->format('Y-m-d') === $transaction_date_raw) {
        $transaction_date = $transaction_date_raw;
    } else {
        echo json_encode(['status' => 'error', 'message' => 'Invalid transaction_date. Expected format: Y-m-d.']);
        exit();
    }
}

// #region agent log
tazrim_debug_log('app/api/save_transaction_dated.php:69', 'Shortcut transaction parsed input', [
